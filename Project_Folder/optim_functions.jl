    ...
    if setup == "BabySFC/Generated_Fields" || setup == "PSI/Generated_Fields"
        ...
        for i in 1:length(Bgoals)
            for j in 1:Int64(length(poi)/3)
                poi_val[3*(j-1)+1:3*j,i] = (field_loops(poi_mat[j,:], simpleloops[i], simpleloopscurrents[i], vertex_positions).-Bgoals[i](poi_mat[j,:])) - (field_loops(poi_mat[j,:], simpleloops[i], simpleloopscurrents[i]*0.9, vertex_positions).-Bgoals[i](poi_mat[j,:]))
            end
        end
    else
        poi_mat = transpose(reshape(poi, (3,:)))
        ...
        for i in 1:length(fields_full)
            poi_val[:,i] = (vec(transpose(fields_full[i](poi_mat) - fields_point_9[i](poi_mat)))*10)
        end
    end
    return poi_val
end
...
function load_data(num_fields)
    ...
    for i in 1:num_fields
        input_full = readdlm("$(setup)/P$(i)Field_5050_Full.csv" , ',' , Float64)
        input_point_9 = readdlm("$(setup)/P$(i)Field_5050_Point_9.csv" , ',' , Float64)
...
#function a_k(theta_k, theta_k_minus_1, g_hat_k, g_hat_k_minus_1, k)
#    global a, A, alpha
#    #gamma = (abs(transpose((theta_k - theta_k_minus_1))*(g_hat_k - g_hat_k_minus_1)))/(norm(g_hat_k - g_hat_k_minus_1)^2)
#    gamma = a/((k+A)^alpha)
#    return gamma
#end
...
function SPSA(theta_0, theta_1)
    ...
    return theta_list, g_hat_list, cond_list
end
...
function next_num_sens(poi_0, poi_1, num_sensors)
    ...
    poi_new, grads_new, conds_new = SPSA(theta_0, theta_1)
    return poi_new, grads_new, conds_new
end
